...
| **non_permanent?** |                 `boolean`                  | **(Optional)** If present indicates that a truthy value in `light`, `heavy` and `special` should be treated as a limited time event. The event can be inferred from the `events` array. |
